...
5. **检查 cron 任务的 context 传递**：对每个已有的 cron 任务，检查它的 message 里是否写明了要读哪些项目文件。如果没有 → 更新 cron message，加入 Context 读取步骤（参考 Cron 任务 Message 模板）。

6. **向主人汇报**：完成后给主人一个总结——建了几个项目、更新了几个 cron、有什么需要主人后续补充的信息。
